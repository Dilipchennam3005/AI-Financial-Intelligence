...
function useMarketStatus() {
  ...
  useEffect(() => {
    function calcStatus() {
      ...
      if (day === 0 || day === 6) return { label: "Market Closed", sub: "Weekend", color: "#b94040" };
      if (mins >= 570 && mins < 960)  return { label: "Market Open", sub: "NYSE · 9:30 AM – 4:00 PM ET", color: "#2a7a4b" };
      if (mins >= 240 && mins < 570)  return { label: "Pre-Market", sub: "4:00 AM – 9:30 AM ET", color: "#c07a2a" };
      if (mins >= 960 && mins < 1200) return { label: "After-Hours", sub: "4:00 PM – 8:00 PM ET", color: "#c07a2a" };
      return { label: "Market Closed", sub: "Opens 9:30 AM ET", color: "#b94040" };
    }

    setStatus(calcStatus());
    const timer = setInterval(() => setStatus(calcStatus()), 30000);
...
// ── Price Chart with hover tooltip + time range
function PriceChart({ data, forecast, range }) {
  ...
  const hx = (i, len, xStart = PL, xEnd = W - PR) => xStart + (i / (len - 1)) * (xEnd - xStart);
  const hy = v => PT + chartH - ((v - minV) / rangeV) * chartH;

  const histPts = displayData.map((d, i) => `${hx(i, displayData.length)},${hy(d.Close)}`).join(" ");
  const lastX   = hx(displayData.length - 1, displayData.length);
  const fcPts   = fcData.map((d, i) => `${lastX + (i / (fcData.length - 1 || 1)) * 60},${hy(d.yhat)}`).join(" ");
  const trend   = allPrices[allPrices.length - 1] >= allPrices[0];
  const lineColor = trend ? "#2a7a4b" : "#b94040";
  const yTicks  = [0.2, 0.5, 0.8].map(f => ({ y: PT + chartH * (1 - f), val: (minV + rangeV * f).toFixed(0) }));

  function handleMouseMove(e) {
    const svg  = svgRef.current;
    if (!svg) return;
    const rect = svg.getBoundingClientRect();
    const scaleX = (W + 60) / rect.width;
    const mx   = (e.clientX - rect.left) * scaleX;

    if (mx < PL || mx > W - PR) { setTooltip(null); return; }

    const idx  = Math.round((mx - PL) / chartW * (displayData.length - 1));
    const d    = displayData[Math.max(0, Math.min(idx, displayData.length - 1))];
    if (!d) return;

    const tx = hx(idx, displayData.length);
    const ty = hy(d.Close);
    setTooltip({ x: tx, y: ty, date: d.Date, price: d.Close, volume: d.Volume });
  }

  return (
    <svg ref={svgRef} width="100%" viewBox={`0 0 ${W + 60} ${H}`}
      ...
      {range === '1Y' && fcData.length > 1 && (
        <>
          <line x1={lastX} y1={PT} x2={lastX} y2={PT + chartH} stroke="#c8c0b8" strokeWidth={1} strokeDasharray="4,3" />
          <polyline points={`${displayData.map((d, i) => `${hx(i, displayData.length)},${hy(d.Close)}`).slice(-1)[0]} ${fcPts}`}
      ...
      {tooltip && (
        <>
          <line x1={tooltip.x} y1={PT} x2={tooltip.x} y2={PT + chartH} stroke="#c8c0b8" strokeWidth={1} />
          <circle cx={tooltip.x} cy={tooltip.y} r={4} fill={lineColor} stroke="#fff" strokeWidth={1.5} />
          <rect
            x={tooltip.x > W - 140 ? tooltip.x - 132 : tooltip.x + 8}
            y={tooltip.y - 36}
            width={124} height={50} rx={6}
            fill="#fff" stroke="#e8e4de" strokeWidth={1}
            style={{ filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.08))" }}
          />
          <text x={tooltip.x > W - 140 ? tooltip.x - 68 : tooltip.x + 70} y={tooltip.y - 20}
            textAnchor="middle" fill="#6b6660" fontSize={9} fontFamily="Georgia, serif">{tooltip.date}</text>
          <text x={tooltip.x > W - 140 ? tooltip.x - 68 : tooltip.x + 70} y={tooltip.y - 6}
            textAnchor="middle" fill="#2a2420" fontSize={11} fontWeight="bold" fontFamily="Georgia, serif">${(tooltip.price || 0).toFixed(2)}</text>
          <text x={tooltip.x > W - 140 ? tooltip.x - 68 : tooltip.x + 70} y={tooltip.y + 8}
            textAnchor="middle" fill="#8a7a6a" fontSize={9} fontFamily="Georgia, serif">
            Vol: {tooltip.volume ? (tooltip.volume / 1e6).toFixed(1) + 'M' : 'N/A'}
          </text>
        </>
      )}
    </svg>
...
export default function App() {
  ...
  const handleAnalyze = async () => {
    ...
    setLoading(true); setError(""); setAnalysis(null); setMessages([]); setRange("1Y");
    try {
      ...
      setMessages([{ role: "assistant", content: `I've analyzed ${data.data.name} (${ticker.toUpperCase()}). The ML models have run and the commentary is ready. What would you like to dig into?` }]);
    ...
    <div style={{ minHeight: "100vh", background: "#faf7f4", color: "#2a2420", fontFamily: "Georgia, serif" }}>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ::-webkit-scrollbar { width: 5px; } ::-webkit-scrollbar-track { background: #f0ece6; } ::-webkit-scrollbar-thumb { background: #c8c0b8; border-radius: 3px; }
        ...
        .analyze-btn { background: #3a5a3a; color: #fff; border: none; padding: 13px 28px; border-radius: 8px; cursor: pointer; font-family: Georgia, serif; font-size: 14px; font-weight: bold; transition: background 0.2s; }
        ...
        .range-btn { background: none; border: 1px solid #e8e4de; border-radius: 5px; padding: 4px 10px; font-size: 11px; font-family: Georgia, serif; color: #8a7a6a; cursor: pointer; transition: all 0.15s; }
        .range-btn.active { background: #3a5a3a; color: #fff; border-color: #3a5a3a; font-weight: bold; }
        .range-btn:hover:not(.active) { border-color: #3a5a3a; color: #3a5a3a; }
        ...
        .market-dot { width: 8px; height: 8px; border-radius: 50%; display: inline-block; margin-right: 6px; animation: pulse 2s ease-in-out infinite; }
        @keyframes pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.4; } }
      `}</style>

      {/* Header */}
      <div style={{ background: "#fff", borderBottom: "1px solid #e8e4de", padding: "14px 40px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          {marketStatus && (
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 12, fontWeight: "bold", color: marketStatus.color, display: "flex", alignItems: "center" }}>
                <span className="market-dot" style={{ background: marketStatus.color }} />
                {marketStatus.label}
              </div>
              <div style={{ fontSize: 10, color: "#b0a898", marginTop: 1 }}>{marketStatus.sub}</div>
            </div>
          )}
          <div style={{ fontSize: 10, color: "#b0a898", fontStyle: "italic" }}>Not investment advice</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <h1 style={{ fontSize: 26, fontWeight: "bold", color: "#2a2420", marginBottom: 6, letterSpacing: "-0.5px" }}>Financial Intelligence Platform</h1>
          ...
          <div style={{ display: "flex", gap: 12, maxWidth: 560 }}>
            <input value={ticker} onChange={e => setTicker(e.target.value.toUpperCase())}
              ...
              placeholder="AAPL, JPM, SPY, VFIAX..." maxLength={10}
        ...
        {error && <div style={{ background: "#fff5f5", border: "1px solid #e8c0c0", color: "#8a3030", padding: "12px 16px", borderRadius: 8, marginBottom: 20, fontSize: 13 }}>{error}</div>}
          ...
          <div className="fade-up">
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
              <div>
                ...
                {lastUpdated && (
                  <div style={{ fontSize: 10, color: "#b0a898", marginTop: 3 }}>Last updated: {fmtTime(lastUpdated)}</div>
                )}
              </div>
              <div style={{ textAlign: "right" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 10, marginBottom: 20 }}>
              {[
                ...
                ["Dividend",       d.dividend_yield ? `${(d.dividend_yield*100).toFixed(2)}%` : "—"],
                ["Volume",         d.avg_volume ? `${(d.avg_volume/1e6).toFixed(1)}M` : "—"],
              ].map(([k, v]) => (
                <div key={k} className="kpi">
                  <div style={{ fontSize: 10, color: "#a09890", marginBottom: 5, textTransform: "uppercase", letterSpacing: 1 }}>{k}</div>
                  <div style={{ fontSize: 14, fontWeight: "bold", color: "#2a2420" }}>{v}</div>
                </div>
              ))}
            </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div className="card">
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, flexWrap: "wrap", gap: 8 }}>
                    <div style={{ fontSize: 13, fontWeight: "bold", color: "#4a4040" }}>
                      Price History
                      {range === '1Y' && <span style={{ fontWeight: "normal", color: "#8a7a6a", fontSize: 12 }}> + 30-Day Forecast</span>}
                    </div>
                    <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
                      ...
                      {range === '1Y' && (
                        <span style={{ fontSize: 11, color: f?.direction === "bullish" ? "#2a7a4b" : "#b94040", fontWeight: "bold", marginLeft: 8 }}>
                          ML: {f?.direction} {f?.forecast_change > 0 ? "+" : ""}{fmt(f?.forecast_change)}%
                        </span>
                      )}
                    </div>
                  </div>
                  <PriceChart data={d.price_data} forecast={f} range={range} />
                  <div style={{ fontSize: 10, color: "#b0a898", marginTop: 6, fontStyle: "italic" }}>Hover over chart for price details</div>
                </div>
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px", display: "flex", flexDirection: "column" }}>
                  ...
                  {chatLoading && (
                    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0" }}>
                      ...
                      <div style={{ fontSize: 12, color: "#b0a898", fontStyle: "italic" }}>Analyzing...</div>
                    </div>
                  )}
                  <div ref={chatEndRef} />
                </div>
        ...
        <div style={{ marginTop: 48, paddingTop: 20, borderTop: "1px solid #e8e4de", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ fontSize: 11, color: "#b0a898", fontStyle: "italic" }}>Prophet forecasting · Momentum scoring · Claude AI commentary</div>
        </div>
      </div>
    </div>
  );
